...
async function getData() {
    let { data } = await axios.get (`https://www.freetogame.com/api/filter?tag=3d.mmorpg.fantasy.pvp&home`)
   console.log(data)
  ...
  return (
    ...
    <h3 className='recommandation m-5'>
       ...
       {gamesList.map((games,i) => <div key={i} className='col-md-4'>
        
            <div className='item'>
                <img src={'https://www.freetogame.com/g/540' + games.thumbnail} className='w-100' alt="" />
                <h3>{games.title}</h3>
            </div>
        </div>)}
